...
pub enum DiskAction {
    DiskOp(String, bool),
    PartitionOp(String, u32, Option<String>, bool),
    ...
    egui::Frame::group(ui.style())
        ...
        .show(ui, |ui| {
                ...
                ui.with_layout(egui::Layout::right_to_left(egui::Align::Center), |ui| {
                    if disk.disk_type == DiskType::USBFlash {
                        ...
                        if ui.add_sized(egui::vec2(100.0, 20.0), egui::Button::new("Set Offline")).clicked() {
                            if disk.is_online {
                                action = Some(DiskAction::DiskOp(disk.id.clone(), true));
                            }
                        }
                    } else {
                        ...
                        if ui.add_sized(egui::vec2(100.0, 20.0), egui::Button::new(button_label)).clicked() {
                            if disk.is_online && disk.is_system_disk {
                                action = Some(DiskAction::PendingOffline(disk.id.clone()));
                            } else {
                                action = Some(DiskAction::DiskOp(disk.id.clone(), disk.is_online));
                            }
                        }
                    }
            ...
            show_partition_list(ui, disk, |part_num, letter, is_mount| {
                action = Some(DiskAction::PartitionOp(disk.id.clone(), part_num, letter, is_mount));
            });
        });
