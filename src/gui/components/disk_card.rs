...
pub fn show_disk_card(ui: &mut egui::Ui, disk: &DiskInfo) -> Option<DiskAction> {
    let mut action = None;
            ...
            ui.horizontal(|ui| {
                ...
                ui.with_layout(egui::Layout::right_to_left(egui::Align::Center), |ui| {
                    if disk.disk_type == DiskType::USBFlash {
                        ...
                        if ui
                            .add_sized(egui::vec2(100.0, 20.0), egui::Button::new("Set Offline"))
                            .clicked()
                            && disk.is_online
                        {
                            action = Some(DiskAction::SetOffline {
                        ...
                        if ui
                            .add_sized(egui::vec2(100.0, 20.0), egui::Button::new(button_label))
                            .clicked()
                        {
                            if disk.is_online && disk.is_system_disk {
                                action = Some(DiskAction::ConfirmSystemOffline {
                                    disk_id: disk.id.clone(),
                                });
                                ...
                                action = Some(DiskAction::SetOnline {
                                    disk_id: disk.id.clone(),
                                });
                            }
                        }
